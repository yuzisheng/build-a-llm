...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, use_cache=False, start_pos=0, cache=None):
        ...
        shortcut = x
        x = self.norm1(x)
        x, next_cache = self.att(x, use_cache=use_cache, start_pos=start_pos,
                                 cache=cache)  # Shape [batch_size, num_tokens, emb_size]
...
class GPTModel(nn.Module):
    ...
    def forward(self, in_idx, use_cache=True, cache=None):
